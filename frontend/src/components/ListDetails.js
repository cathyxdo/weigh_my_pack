...
export default function ListDetails({
  ...
  function handleCopyLink(event) {
    ...
    navigator.clipboard.writeText(
      process.env.REACT_APP_SHARE_PAGE_BASE_URL + list.id
    );
    ...
    function updateList(categoryToAdd) {
      return apiList.map((list, index) => {
        if (index === selectedIndex) {
          list.categories = [...list.categories, categoryToAdd];
          return list;
        } else {
          return list;
        }
  ...
  function handleNameChangeSubmit(event) {
    ...
    if (isLoggedIn) {
      axiosInstance
        .patch(process.env.REACT_APP_API_BASE_URL + "lists/" + list.id + "/", {
          name: listName,
        })
        .then((result) => {
  ...
  return (
    <div className={"listDetails " + (showSideBar ? "showBar" : "hideBar")}>
      ...
      {!isLoggedIn && (
        <div className="fyiMessage">
          FYI all of your lists will be stored on your computer. Log in or sign
          up for an account to be able to share your list with others!
        </div>
      )}
      {!list && (
        <div>
          ...
          <p>
            Create your first backpacker supply list by clicking{" "}
            <em>"+ Add List"</em>{" "}
          </p>

          <p>
            From there you can start creating categories and adding the items in
            your pack!
          </p>
        </div>
      )}
      ...
      {list && (
        <div>
          ...
          <header>
            {!isEditing && (
              <h2>
                <span className="list-details-name" onClick={setIsEditing}>
                  {list.name}
                </span>
              </h2>
            )}
